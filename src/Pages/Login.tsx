...
function Login() {
  ...
  useEffect(() => {
    if (!document.title.includes("Login")) {
      document.title = "HRDirect - Login";
    }
  }, []);

  return (
    <div className="border-2 border-gray-400/50 bg-white/10 rounded-2xl flex flex-col items-center gap-8">
      <h1 className="bg-black/40 text-3xl border-b-2 rounded-2xl pb-4 pt-2 text-center border-gray-400/70 w-full font-extrabold underline">
        Login
      </h1>
      <p>Please enter username and password.</p>
      <form
        onSubmit={handleSubmit}
        className="flex flex-row gap-4 pb-4 px-4 pt-4"
      >
        <input
